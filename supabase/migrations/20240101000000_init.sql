...
-- These are Supabase-specific and not managed by Prisma
-- ============================================================
...
-- Job images: public read, authenticated write
...
-- Tradie avatars: public read, owner write
...
CREATE POLICY "Tradies can update their own avatar"
  ON storage.objects FOR UPDATE
  USING (bucket_id = 'tradie-avatars' AND (storage.foldername(name))[1] = auth.uid()::text);

-- Portfolio images: public read, authenticated write
...
CREATE POLICY "Tradies can delete their own portfolio images"
  ON storage.objects FOR DELETE
  USING (bucket_id = 'portfolio-images' AND (storage.foldername(name))[1] = auth.uid()::text);

-- Tradie documents: private, owner-only
CREATE POLICY "Tradies can view their own documents"
  ON storage.objects FOR SELECT
  USING (
...
-- ============================================================
CREATE INDEX IF NOT EXISTS idx_jobs_suburb_trgm          ON jobs             USING gin (suburb gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_jobs_description_trgm     ON jobs             USING gin (description gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_tradie_suburb_trgm        ON tradie_profiles  USING gin ("primarySuburb" gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_users_email_trgm          ON users            USING gin (email gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_seo_pages_slug_trgm       ON seo_pages        USING gin (slug gin_trgm_ops);
