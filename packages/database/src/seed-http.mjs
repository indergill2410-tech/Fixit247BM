...
async function seedCustomers() {
  ...
  for (const [i, c] of customers.entries()) {
    ...
    let { data: user } = await sb.from('users').select('id').eq('email', c.email).maybeSingle();
    if (!user) {
      const { data: created, error: ue } = await sb.from('users').insert({
        id: randomUUID(), email: c.email,
        first_name: c.firstName, last_name: c.lastName,
        phone: c.phone, role: 'CUSTOMER', is_active: true,
        email_verified: new Date().toISOString(),
      }).select('id').single();
      if (ue) { console.log(`FAIL (user insert: ${ue.message})`); continue; }
      user = created;
    } else {
      await sb.from('users').update({ first_name: c.firstName, last_name: c.lastName, phone: c.phone }).eq('id', user.id);
    }
    const uid = user.id;
...
async function seedTradies() {
  ...
  for (const [i, t] of tradies.entries()) {
    ...
    // User row — find or create
    let { data: user } = await sb.from('users').select('id').eq('email', t.email).maybeSingle();
    if (!user) {
      const { data: created, error: ue } = await sb.from('users').insert({
        id: randomUUID(), email: t.email,
      ...
      await sb.from('users').update({ first_name: t.firstName, last_name: t.lastName, phone: t.phone }).eq('id', user.id);
    }
    const uid = user.id;
