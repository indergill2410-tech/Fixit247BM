...
async function seedDemoUsers() {
  ...
  const adminAuthId = await ensureAuthUser(admin, 'admin@demo.fixit247.com.au', {
    firstName: 'Super', lastName: 'Admin', role: 'SUPER_ADMIN', onboardingComplete: true,
  }, DEMO_PASSWORD);
  await db.user.upsert({
    where: { email: 'admin@demo.fixit247.com.au' },
    create: {
      ...(adminAuthId ? { id: adminAuthId } : {}),
      email: 'admin@demo.fixit247.com.au', firstName: 'Super', lastName: 'Admin',
      role: 'SUPER_ADMIN', isActive: true, emailVerified: new Date(), onboardingComplete: true,
    },
    update: { firstName: 'Super', lastName: 'Admin', isActive: true, role: 'SUPER_ADMIN', onboardingComplete: true },
  });
  console.log('✅ Super admin demo account seeded');
  ...
  console.log('   Super Admin: admin@demo.fixit247.com.au');
}
