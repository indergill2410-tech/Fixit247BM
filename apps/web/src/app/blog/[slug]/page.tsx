  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="mx-auto max-w-3xl px-4 py-12">
        <Link href="/blog" className="mb-8 flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
          <ChevronLeft size={14} /> Back to blog
        </Link>
        <div className="mb-4 flex items-center gap-3">
          <span className="rounded-full bg-red-100 px-3 py-1 text-xs font-semibold text-red-700">{article.category}</span>
          <span className="flex items-center gap-1 text-xs text-gray-400"><Clock size={12} />{article.readTime} read</span>
        </div>
        <h1 className="mb-6 text-3xl font-extrabold text-gray-900 leading-tight">{article.title}</h1>
        <div className="prose prose-gray max-w-none">
          {paragraphs.map((block, i) => {
            if (block.type === 'h2') return <h2 key={i} className="mt-8 mb-3 text-xl font-bold text-gray-900">{block.text}</h2>;
            if (block.type === 'ul') return <ul key={i} className="mb-4 space-y-1 pl-4">{(block as any).items?.map((item: string, j: number) => <li key={j} className="text-gray-700 text-sm list-disc">{item}</li>)}</ul>;
            return <p key={i} className="mb-4 text-gray-700 leading-relaxed">{block.text}</p>;
          })}
        </div>
        <div className="mt-10 rounded-2xl bg-gray-900 p-6 text-white text-center">
          ...
          <Link href="/emergency" className="inline-block rounded-xl bg-red-600 px-6 py-3 font-bold text-white hover:bg-red-700 transition-colors">
