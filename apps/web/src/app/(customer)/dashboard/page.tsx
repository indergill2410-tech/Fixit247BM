...
export default async function CustomerDashboardPage() {
  ...
  return (
    <DashboardShell role="CUSTOMER">
      {/* Emergency banner */}
      <div className="mb-6 flex items-center justify-between rounded-2xl bg-gradient-to-r from-red-600 to-red-500 p-5 text-white shadow-lg">
        <div>
          <p className="text-sm font-medium text-red-100">Need urgent help right now?</p>
          <p className="mt-0.5 text-xl font-bold">Emergency tradie dispatch — available 24/7</p>
        </div>
            ...
            <div className="mt-3 space-y-2">
              {[
                ...
                <Link
                  key={action.href}
                  href={action.href}
                  className="flex items-center justify-between rounded-xl p-3 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors"
                >
                  {action.label}
